...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    axios.get("/api/search?q=".concat(this.value)).then(function (res) {
      if (res.data.length) {
        searchResults.innerHTML = dompurify__WEBPACK_IMPORTED_MODULE_0___default().sanitize(searchResultsHTML(res.data));
        return;
      }

      searchResults.innerHTML = dompurify__WEBPACK_IMPORTED_MODULE_0___default().sanitize("<div class=\"search__result\">No results for ".concat(_this.value, " found</div>"));
    }).catch(function (err) {
      console.error(err);
    });
  });
